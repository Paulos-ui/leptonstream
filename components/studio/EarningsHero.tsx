...
export default function EarningsHero({
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl border border-ink/10 bg-gradient-to-b from-white/60 to-transparent p-8 sm:p-10">
      {!reduce && (
        <motion.div
          key={pulse}
          aria-hidden
          ...
          style={{ background: "radial-gradient(circle, rgba(91,160,19,0.5), transparent 70%)" }}
        />
      )}
      <div className="relative flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="font-mono text-[10px] uppercase tracking-eyebrow text-muted">available to withdraw</div>
          <motion.div
            animate={pulse && !reduce ? { scale: [1, 1.025, 1] } : {}}
            transition={{ duration: 0.5 }}
            className="mt-2 font-serif text-6xl tabular-nums text-leaf sm:text-7xl"
          >
            ${display.toFixed(6)}
          </motion.div>
        </div>
        <div className="text-right font-mono text-[11px] text-muted">
          <div className="text-ink/70">{supporters} supporter{supporters === 1 ? "" : "s"}</div>
